...
let write_groupls_t : _ -> groupls_t -> _ = (
  fun ob x ->
...
let string_of_groupls_t ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_groupls_t ob x;
...
let read_groupls_t = (
  fun p lb ->
    ...
    let field_isAdminGroup = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    let field_isRootGroup = ref (Obj.magic (Sys.opaque_identity 0.0)) in
    ...
    with Yojson.End_of_object -> (
        if !bits0 <> 0x7f then Ag_oj_run.missing_fields p [| !bits0 |] [| "authorityType"; "displayName"; "fullName"; "isAdminGroup"; "isRootGroup"; "shortName"; "url" |];
        (
          {
...
let groupls_t_of_string s =
  read_groupls_t (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
let write__1 = (
...
let string_of_lsGroup ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_lsGroup ob x;
  Bi_outbuf.contents ob
let read_lsGroup = (
...
let lsGroup_of_string s =
  read_lsGroup (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
