...
let read_capabilities_t = (
  fun p lb ->
    ...
    let field_isAdmin = ref (Obj.magic 0.0) in
    let field_isGuest = ref (Obj.magic 0.0) in
    let field_isMutable = ref (Obj.magic 0.0) in
    let bits0 = ref 0 in
    try
      Yojson.Safe.read_space p lb;
...
let read_userInfo = (
  fun p lb ->
    ...
    let field_capabilities = ref (Obj.magic 0.0) in
    let bits0 = ref 0 in
    try
      Yojson.Safe.read_space p lb;
      Yojson.Safe.read_object_end lb;
