...
let write_people_t : _ -> people_t -> _ = (
  fun ob x ->
...
let string_of_people_t ?(len = 1024) x =
...
let read_people_t = (
  fun p lb ->
        ...
        fun s pos len ->
          ...
          match len with
            ...
            | 12 -> (
                match String.unsafe_get s pos with
                  | 'c' -> (
                      if String.unsafe_get s (pos+1) = 'o' && String.unsafe_get s (pos+2) = 'm' && String.unsafe_get s (pos+3) = 'p' && String.unsafe_get s (pos+4) = 'a' && String.unsafe_get s (pos+5) = 'n' && String.unsafe_get s (pos+6) = 'y' && String.unsafe_get s (pos+7) = 'e' && String.unsafe_get s (pos+8) = 'm' && String.unsafe_get s (pos+9) = 'a' && String.unsafe_get s (pos+10) = 'i' && String.unsafe_get s (pos+11) = 'l' then (
                        17
                      )
                      else (
                        -1
    ...
    with Yojson.End_of_object -> (
        if !bits0 <> 0x3ffffff then Ag_oj_run.missing_fields p [| !bits0 |] [| "url"; "userName"; "enabled"; "firstName"; "lastName"; "jobtitle"; "organization"; "location"; "telephone"; "mobile"; "email"; "companyaddress1"; "companyaddress2"; "companyaddress3"; "companypostcode"; "companytelephone"; "companyfax"; "companyemail"; "skype"; "instantmsg"; "userStatus"; "userStatusTime"; "googleusername"; "quota"; "sizeCurrent"; "persondescription" |];
        (
          {
            url = !field_url;
...
let people_t_of_string s =
  read_people_t (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
let write__1 = (
...
let write_userls : _ -> userls -> _ = (
  fun ob x ->
    ...
    if !is_first then
      ...
      Bi_outbuf.add_char ob ',';
    Bi_outbuf.add_string ob "\"people\":";
    (
      write__1
    )
      ob x.people;
...
let string_of_userls ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_userls ob x;
  Bi_outbuf.contents ob
let read_userls = (
    ...
    try
      ...
      let f =
        fun s pos len ->
          if pos < 0 || len < 0 || pos + len > String.length s then
            invalid_arg "out-of-bounds substring position or length";
          if len = 6 && String.unsafe_get s pos = 'p' && String.unsafe_get s (pos+1) = 'e' && String.unsafe_get s (pos+2) = 'o' && String.unsafe_get s (pos+3) = 'p' && String.unsafe_get s (pos+4) = 'l' && String.unsafe_get s (pos+5) = 'e' then (
            0
          )
          else (
            -1
      ...
      assert false;
    with Yojson.End_of_object -> (
        if !bits0 <> 0x1 then Ag_oj_run.missing_fields p [| !bits0 |] [| "people" |];
        (
          {
            people = !field_people;
          }
...
let userls_of_string s =
  read_userls (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
